...
export default function LawyerAnalyticsPage() {
  ...
  const { caseload, tasks, research, documents, workload, priorities } = data;
                ...
                <tbody className="text-xs font-semibold text-[#21170F]">
                  ...
                  {workload.map((cw: any) => (
                    <tr key={cw.id} className="border-b border-[#E2D5C1]/20">
                      ...
                      <td className="py-3">
                        <Link href={`/lawyer/cases/detail?id=${encodeURIComponent(cw.id)}`} className="text-[#A66A22] hover:underline text-[10px] font-bold">
                          View Workspace
                        </Link>
                      </td>
                    </tr>
